...
class _AdminHomeState extends State<AdminHome> {
  ...
  void addgener(String gener) {
    FirebaseFirestore.instance.collection('metadata').doc('genres').update({
  ...
  void addCode() {
    ...
    DocumentReference<Map<String, dynamic>> couponDataRef = FirebaseFirestore
        .instance
        .collection('metadata')
  ...
  void setLang() async {
    ...
    DocumentReference appLangRef =
        firestore.collection('metadata').doc('content_lang');

    appLangRef.update({'app_lang': LangData.appLang});

    DocumentReference contentLangRef =
        firestore.collection('metadata').doc('content_lang');

    contentLangRef.update({'lang_list': LangData.ContentLang});

    DocumentReference LangNameRef =
        firestore.collection('metadata').doc('content_lang');

    LangNameRef.update({'Lang_name': LangData.LangName});

    DocumentReference viocelistRef =
        firestore.collection('metadata').doc('content_lang');
  ...
  Future<void> mailsend(String file_url) async {
    ...
    QuerySnapshot user_list_snap = await db.collection('users').get();

    final List<String> user_list =
        user_list_snap.docs.map((doc) => doc['email'] as String).toList();
